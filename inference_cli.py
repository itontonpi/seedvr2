...
def _stream_video_chunks(
    ...
    while frames_read < frames_to_process:
        ...
        if streaming:
            clear_memory(debug=debug, deep=False, force=True, timer_name=cleanup_timer_name)
...
def _process_frames_core(
    ...
    # Pipeline mode: process each batch through all phases with GPU-CPU overlap
    if getattr(args, 'pipeline', False):
        if getattr(args, 'pipeline_mode', 'legacy') == 'parallel':
            from src.core.pipeline_parallel import process_batches_parallel
            ctx = process_batches_parallel(
        ...
        # Sequential mode: all batches per phase (original behavior)
        # Phase 1: Encode
...
"""
    ...
    perf_group.add_argument("--pipeline", action="store_true",
                        help="Enable pipelined batch processing for maximum GPU utilization. "
...
def main() -> None:
    ...
    # Persist torch.compile cache across restarts (critical for serverless cold starts)
    if args.compile_dit or args.compile_vae:
        compile_cache = os.path.join(
            args.model_dir or f"./models/{SEEDVR2_FOLDER_NAME}",
            ".torch_compile_cache"
        )
        os.makedirs(compile_cache, exist_ok=True)
        os.environ.setdefault("TORCHINDUCTOR_CACHE_DIR", compile_cache)
        debug.log(f"torch.compile cache: {compile_cache}", category="setup")

...
class SeedVR2VideoUpscaler(io.ComfyNode):
    ...
    def execute(cls, image: torch.Tensor, dit: Dict[str, Any], vae: Dict[str, Any], 
        ...
        try:
            ...
            # Auto-detect pipeline mode: use when both models on same CUDA device,
            # no BlockSwap, and sufficient for zero-swap processing
            use_pipeline = (
                dit_device == vae_device 
                and dit_device.type == 'cuda' 
                and block_swap_config is None
                and dit_offload_device is None 
                and vae_offload_device is None
            )
            
            if use_pipeline:
                from ..core.pipeline import process_batches_pipelined
                debug.log("Using pipeline mode (both models on same GPU, zero model swaps)", 
                         category="generation", force=True)
                ctx = process_batches_pipelined(
                    runner, ctx=ctx, images=image,
                    debug=debug,
                    batch_size=batch_size,
                    uniform_batch_size=uniform_batch_size,
                    seed=seed,

...
def cleanup_dit(runner: Any, debug: Optional['Debug'] = None, cache_model: bool = False) -> None:
    ...
    try:
        ...
        if param_device.type not in ['meta', 'cpu']:
            # When caching with no offload device, keep model on GPU
            # (optimal for pipeline mode / serverless where VRAM is plentiful)
            if cache_model:
                offload_target = getattr(runner, '_dit_offload_device', None)
                if offload_target is None or str(offload_target) == 'none':
                    if debug:
                        debug.log("DiT staying on GPU (no offload device, cache enabled)", category="cleanup")
                else:
    ...
    try:
        ...
        if param_device.type not in ['meta', 'cpu']:
            ...
            if cache_model:
                ...
                if offload_target is None or str(offload_target) == 'none':
                    if debug:
                        debug.log("VAE staying on GPU (no offload device, cache enabled)", category="cleanup")
                else:
